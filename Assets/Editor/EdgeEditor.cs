...
public class EdgeEditor : Editor {
    ...
    public override void OnInspectorGUI(){
        ...
        if (ReferenceEquals(edge, null)) return;

        if (edge.Graph is{IsLocked: true}){
            GUILayout.Label("The edge is locked because its graph is.");
            GUILayout.Label("locked. Unlock the graph to edit.");
        }
        else if (edge.EdgeCollection is{IsLocked: true}){
        ...
        if (ReferenceEquals(edge.Graph, null)) return;

        if (GUILayout.Button("Edit graph")) Selection.activeGameObject = edge.Graph.gameObject;
