...
public class Agent : MovingEntity
{
    ...
    public PathManager PathManager 
    {
        get 
        {
            return PathPlanner.PathManager;
        }
    }
    ...
    {
        float angle = Vector3.Angle(targetPosition - Kinematic.Position, transform.forward);
    ...
    {
        return CanMoveBetween(Kinematic.Position, endPosition, satisfactionRadius, PathManager.PathObstaclesLayerMask);
    }
    ...
    {
        return CanMoveBetween(startPosition, endPosition, PathManager.CloseEnoughDistance, PathManager.PathObstaclesLayerMask);
    }
    ...
    public bool CanMoveBetween(Vector3 startPosition, Vector3 endPosition, float satisfactionRadius, LayerMask obstacleLayers)
    {
        ...
        Vector3 direction = (endPosition - startPosition) / distance;
    ...
    {
        return CanStepRight(PathManager.CloseEnoughDistance, PathManager.PathObstaclesLayerMask, out positionOfStep);
    }
    ...
    {
        float stepDistance = Kinematic.Radius * 2;
    ...
    {
        float angle = Vector3.Angle(Kinematic.Position, target);
    ...
    {    
        DamageInflictedEventPayload payload = eventArg.EventData;
        ...
        if (IsDead)
        {
            EventManager.Instance.Enqueue<EntityDestroyedEventPayload>(
                Events.EntityDestroyed,
    ...
    {    
        EntityDestroyedEventPayload payload = eventArg.EventData;
    ...
    {    
        WeaponSoundEventPayload payload = eventArg.EventData;
