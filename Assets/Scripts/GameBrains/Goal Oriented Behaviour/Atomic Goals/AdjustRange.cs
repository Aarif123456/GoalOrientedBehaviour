namespace GameBrains.AI
{
    ...
    public sealed class AdjustRange : Goal
    {
        ...
        public override StatusTypes Process()
        {
            ...
            {
                float distanceFromTarget = Vector3.Distance(Agent.Kinematic.Position, targetAgent.Kinematic.Position);
                ...
                {
                    Vector3 idealPosition = targetAgent.Kinematic.Position;
                
                    if (Mathf.Approximately(distanceFromTarget, 0))
                    {
                        Vector2 aDirection = Random.onUnitSphere;
                        idealPosition += new Vector3(aDirection.x, Agent.Kinematic.Position.y, aDirection.y) * IdealRange;
                    }
                    else
                    {
                        idealPosition += (Agent.Kinematic.Position - targetAgent.Kinematic.Position).normalized * IdealRange;
                    }
